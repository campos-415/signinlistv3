...
import { daysLeft, eligiblePackagesFor, packageLabel } from "@/lib/clients";
...
export default function StaffCheckIn({ onDone }: { onDone?: () => void }) {
  ...
  function serviceFor(dog: Client, open: OpenVisit | undefined): ServiceType {
  ...
  function packagesFor(dog: Client): Package[] {
    return ctx ? eligiblePackagesFor(ctx.packages, phone.trim(), dog.dog_name) : [];
  }

  function chosenPackage(dog: Client): Package | null {
    const eligible = packagesFor(dog);
    const id = dog.id ? packageByDog[dog.id] : undefined;
    return (id && eligible.find((p) => p.id === id)) || eligible[0] || null;
  }

  async function submit(dog: Client, action: SignAction) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-[11px] text-slate-400">Client phone number</label>
          <input
            value={phone}
        ...
        <div>
          <label className="mb-1 block text-[11px] text-slate-400">Your name (recorded on the visit)</label>
          <input
            value={staffName}
      ...
      {loading && <p className="mt-2 text-xs text-slate-400">Looking up…</p>}
      ...
      {dogs.length > 0 && (
        <div className="mt-4 space-y-3">
          {dogs.map((dog) => {
            ...
            const eligible = packagesFor(dog);
            const pkg = chosenPackage(dog);
            const willUsePackage = isIn && packageApplies(service, open, pkg, now);
            const addons = (dog.id && addonsByDog[dog.id]) || [];
            const reservation = activeBoardingFor(dog);

            return (
              <div key={dog.id} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                ...
                {!isIn && (
                  <>
                    {!reservation && (
                      <div className="mb-2">
                        <label className="mb-1 block text-[11px] text-slate-400">Service</label>
                    ...
                    <label className="mb-1 block text-[11px] text-slate-400">Add-ons</label>
                    <div className="flex flex-wrap gap-1.5">
                      {ADDONS.map((a) => (
                        <button
                          key={a.key}
                ...
                {/* Package choice — staff-only, which is why it lives here
                    and not on the parent-facing kiosk. Only a pick-up
                    actually spends a day. */}
                {pkg && (
                  <div className="mt-2">
                    <label className="mb-1 block text-[11px] text-slate-400">
                      Package {eligible.length > 1 ? "to draw from" : ""}
                    </label>
                    {eligible.length > 1 ? (
                      <select
                        value={pkg.id ?? ""}
                        onChange={(e) =>
                          dog.id && setPackageByDog((prev) => ({ ...prev, [dog.id!]: e.target.value }))
                        }
                        className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs outline-none focus:border-accent-500"
                      >
                        {eligible.map((p) => (
                      ...
                      <span className="text-xs text-slate-600">📦 {packageLabel(pkg)}</span>
                    )}
                    {isIn && (
                      <p className="mt-1 text-[11px] text-slate-400">
                        {willUsePackage
                          ? "Signing out now spends one day from this package."
                          : service !== "daycare"
